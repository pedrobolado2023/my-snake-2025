...
class Snake {
    ...
    render(ctx, camera) {
        ...
        if (this.isPlayer || camera.zoom > 0.6) {
            ...
            ctx.strokeStyle = this.skin.colors[0];
        ...
        for (let i = this.segments.length - 1; i >= 0; i -= step) {
            ...
            // Escolha de cor baseada no padrão
            let c1 = this.skin.colors[0];
            let c2 = this.skin.colors[1] || Utils.darkenColor(c1, 40);
            if (['stripes', 'rainbow'].includes(this.skin.pattern)) {
                const idx = Math.floor(i / 8) % this.skin.colors.length;
                c1 = this.skin.colors[idx];
